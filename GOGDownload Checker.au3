...
Global $textcol, $top, $tot, $unrar, $update, $version, $zipcheck
...
Func ImitationConsoleGUI($start)
	...
	$height = 590
	$width = 710
	$ConsoleGUI = GuiCreate("GOGDownload Checker " & $version & " - Console" & $shutwarn, $width, $height, @DesktopWidth - $width - 28, 30, $WS_OVERLAPPED + _
	...
	GUICtrlSetTip($Checkbox_zip, "Compressed files (ZIP, 7Z, BZ2, PK4 only)!")
...
;~ 		EndIf
	...
	While 1
		...
		Case $msg = $Checkbox_zip
			; Compressed files (ZIP, 7Z, BZ2, PK4 only)
						...
						If FileExists($path) Then
							...
							ElseIf $fext = ".rar" And GUICtrlRead($Checkbox_rar) = $GUI_CHECKED Then
								...
								Else
									$err = 4
								EndIf
							ElseIf ($fext = ".zip" Or $fext = ".7z" Or $fext = ".bz2" Or $fext = ".pk4") And GUICtrlRead($Checkbox_zip) = $GUI_CHECKED Then
								GetFileSize()
								If $zipcheck = 1 Then
									$zip = 1
									FileChangeDir(@ScriptDir & "\7-Zip")
...
Func AddFileToList()
	...
	If $fext = ".exe" Or $fext = ".zip" Or $fext = ".7z" Or $fext = ".rar" Or $fext = ".sh" Or $fext = ".bz2" Or $fext = ".pk4" Or $fext = ".pdf" Then
	...
	Else
		MsgBox(262192, "File Error", "Only BZ2 & EXE & PDF & PK4 & RAR & SH & ZIP & 7Z are currently supported!", 0, $DropboxGUI)
